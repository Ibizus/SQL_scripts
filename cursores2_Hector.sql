...
/* Realizar mediante un cursor un procedimiento que se recorra todos los libros de su tabla y para cada libro de la editorial 'Paidos' hará una copia de dicho registro sobre una nueva tabla llamada  'libros_de_paidos'.
En caso de no pertenecer a paidos hará una copia de dicho registro sobre una nueva tabla llamada libros_no_paidos, pero no insertará el código de editorial en este caso */
...
-- DECLARE... 
  ...
  DECLARE var_nombre varchar(20);
  DECLARE cursor_libros CURSOR FOR 
  SELECT l.codigo, l.titulo, l.codigoeditorial, l.precio, e.nombre 
  FROM libros as l join editoriales as e on e.codigo = l.codigoeditorial;
  
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET var_final = 1;
  
  OPEN cursor_libros;
  bucle: LOOP
    FETCH cursor_libros INTO var_codigo, var_titulo, var_editorial, var_precio, var_nombre;
    IF var_final = 1 THEN
      LEAVE bucle;
    END IF;
    
    if var_nombre = 'Paidos' then
    	INSERT into libros_paidos values (var_codigo, var_titulo, var_editorial, var_precio);
    else
    	INSERT into libros_no_paidos values (var_codigo, var_titulo, var_precio);
    end if;
